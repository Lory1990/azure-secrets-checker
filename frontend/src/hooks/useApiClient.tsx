...
import { IClientRequestData, IToken } from "../api/apiClient"
        ...
        if (activeMsalAccount) {
            try {
                ...
                if (error instanceof Error && error.name === "InteractionRequiredAuthError") {
                    ...
                    console.error("[getToken] No authentication method available to get token", { token, authenticatedMSALAccounts: msalAuth.accounts })
                }
            }
        } else {
            console.error("No authentication method available to get token", { token, authenticatedMSALAccounts: msalAuth.accounts, msalInstance: msalAuth.instance, maslAccountsFromRequest: msalAuth.instance.getAllAccounts() })
        }
    }
